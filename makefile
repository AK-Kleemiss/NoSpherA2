...
ifeq ($(NAME),WINDOWS)
	@if not exist featomic\featomic_install\lib\metatensor.lib ( \
		...
		cd $(MAKEFILE_DIR)/featomic\featomic && if not exist build mkdir build && cd build && cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DFEATOMIC_FETCH_METATENSOR=ON -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=../../featomic_install --fresh .. && make install \
	) else ( \
