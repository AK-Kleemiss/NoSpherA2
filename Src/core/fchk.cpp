...
#pragma omp parallel for
				...
				for (int a = 0; a < wave.get_ncen(); a++)
				{
						...
						for (int s = 0; s < wave.get_atom_shell_count(a); s++)
						{
								...
								if (debug)
								{
										file << "Shell: " << s << " of atom: " << a << " Shell type: " << type_temp << "\n"
												 << "start: " << wave.get_shell_start(a, s)
												 << " stop: " << wave.get_shell_end(a, s) << "\n"
												 << "factor: ";
								}
								switch (type_temp)
								{
										...
										for (int i = wave.get_shell_start(a, s); i <= wave.get_shell_end(a, s); i++)
										{
												if (debug)
												{
														file << "Contraction coefficient before: " << wave.get_atom_basis_set_coefficient(a, i)
																 << " Contraction coefficient after:  " << factor * wave.get_atom_basis_set_coefficient(a, i) << "\n";
												}
								...
								if (debug)
										file << "This shell has: " << wave.get_shell_end(a, s) - wave.get_shell_start(a, s) + 1 << " primitives\n";
						}
				}
				//-----------debug output---------------------------------------------------------
				if (debug)
								...
								for (int s = 0; s < wave.get_atom_shell_count(a); s++)
								{
										...
										case 1:
												...
												if (debug && wave.get_atom_shell_primitives(a, s) != 1 && m == 0)
														file << "Pushing back 1 coefficient for S shell, this shell has " << wave.get_atom_shell_primitives(a, s) << " primitives! Shell start is: " << wave.get_shell_start(a, s) << "\n";
												break;
										case 2:
												for (int i = 0; i < 3; i++)
														CMO.push_back(changed_coefs[m][wave.get_shell_start_in_primitives(a, s) + i]);
												if (debug && wave.get_atom_shell_primitives(a, s) != 1 && m == 0)
														file << "Pushing back 3 coefficients for P shell, this shell has " << wave.get_atom_shell_primitives(a, s) << " primitives!\n";
												break;
										case 3:
												for (int i = 0; i < 6; i++)
														CMO.push_back(changed_coefs[m][wave.get_shell_start_in_primitives(a, s) + i]);
												if (debug && wave.get_atom_shell_primitives(a, s) != 1 && m == 0)
														file << "Pushing back 6 coefficient for D shell, this shell has " << wave.get_atom_shell_primitives(a, s) << " primitives!\n";
												break;
										case 4:
												for (int i = 0; i < 10; i++)
										...
										case 5:
												for (int i = 0; i < 15; i++)
														CMO.push_back(changed_coefs[m][wave.get_shell_start_in_primitives(a, s) + i]);
												if (debug && wave.get_atom_shell_primitives(a, s) != 1 && m == 0)
														file << "Pushing back 15 coefficient for G shell, this shell has " << wave.get_atom_shell_primitives(a, s) << " primitives!\n";
												break;
										}
										run_2++;
								}
								if (debug && m == 0)
										file << "finished with atom!\n";
						}
						if (debug)
								file << "finished with MO!\n";
						if (nshell != run_2)
								nshell = run_2;
				}
				if (write_beta)
				{
					...
					continue;
								for (int a = 0; a < wave.get_ncen(); a++)
								{
										...
										{
												if (debug)
														file << "Going to load the " << wave.get_shell_start_in_primitives(a, s) << ". value\n";
												switch (wave.get_shell_type(a, s))
												{
												case 1:
...
#pragma omp parallel for
						for (int iv = 0; iv <= iu; iv++)
						{
								...
								for (int m = 0; m < wave.get_nmo(); m++)
								{
										...
										if (wave.get_MO_op(m) == 0)
												...
												temp = occ * CMO_beta[iu + ((m % nao) * nao)] * CMO_beta[iv + ((m % nao) * nao)];
	...
	else err_checkf(false, "Unsupported wavefunction type: " + std::to_string(wave.get_origin()), file);
		if (debug)
