...
void printCUDA(const cudaDeviceProp* prop, const int nDevices, ostream& file)
{
	...
	for (int dev = 0; dev < nDevices; dev++) {
		file << "Device Number: " << dev << "\n";
		file << "- Device name: " << (prop[dev]).name << "\n";
		file << "- Max global memory   = " << prop[dev].totalGlobalMem / 1048576.0f << " MBytes\n";
		file << "- Max constant memory = " << prop[dev].totalConstMem / 1048576.0f << " MBytes\n";
		file << "- Capability : " << prop[dev].major << "." << prop[dev].minor << "\n";
		file << "- Total global memory: " << (float)prop[dev].totalGlobalMem / 1048576.0f << " MByte\n";
...
#pragma omp parallel for
	...
	if (debug)
	{
		...
		for (int prototype = 0; prototype < Prototype_grids.size(); prototype++)
			file << "Number of gridpoints for atom type " << atom_type_list[prototype] << ": " << Prototype_grids[prototype].get_num_grid_points() << "\n";
	...
	if (debug)
	{
		for (int i = 0; i < atom_type_list.size(); i++)
		{
			...
			file << "Number of radial density points for atomic number " << atom_type_list[i] << ": " << radial_density[i].size() << "\n";
...
#pragma omp parallel for
	...
	for (int i = 0; i < asym_atom_list.size(); i++)
	{
		...
		gridSize = (MaxGrid + blocks - 1) / blocks;
		if (debug)
			file << i << ": num points: " << num_points[i] << " blocks: " << gridSize << " threads: " << blocks << " grid > points? " << (blocks * gridSize > num_points[i]) << "\n";
		int type;
		for (int j = 0; j < atom_type_list.size(); j++)
	...
	if (per_MO)
	{
		...
		for (int mo = 0; mo < nmo_temp; mo++)
		{
			...
			if (debug)
				file << "MO " << mo << " starting at coef: " << nex_offset << "\n";
