			...
			if (neighbours[a][b]) {
				if (vec_length({ wfn.get_atom_coordinate(b, 0) - rijx2,
						...
						if (extrema.size() == 1 && extrema[0].type == DensityExtremum::Type::Minimum) {
							...
							if (maxmin > 0) {
								...
								if (extrema[1].type == DensityExtremum::Type::Minimum)
									...
									if (debug)
										std::cout << "Strange case with 3 extrema but middle is not a minimum. Using Maximum.\n";
									use_extr = 1;
								}
							}
						}
							...
							if (extrema[0].type == DensityExtremum::Type::Minimum && extrema[1].type == DensityExtremum::Type::Maximum) {
								...
								std::cout << "WARNING: Unexpected types of extrema found between atoms " << a << " and " << b << ". Setting chi to 1.0.\n";
								chi[a * ncen + b] = 1.0;
								chi[b * ncen + a] = 1.0;
								continue;
							}
							...
							std::cout << "WARNING: Unexpected number of extrema (" << extrema.size() << ") found between atoms " << a << " and " << b << ". Setting chi to 1.0.\n";
							chi[a * ncen + b] = 1.0;
							chi[b * ncen + a] = 1.0;
							continue;
						}
					...
					if (extrema[use_extr].distB == 0.0) {
						std::cout << "WARNING: Division by zero detected for chi between atoms " << a << " and " << b << ". Setting chi to 1.0.\n";
						chi[a * ncen + b] = 1.0;
						chi[b * ncen + a] = 1.0;
					}
					else {
					...
					if (ri >= 0.90 || ri <= 0.10) {
						std::cout << "WARNING: Large bond polarization detected between atoms " << a << " and " << b
							<< " (ri = " << ri << ", ri >= 0.90 or ri <= 0.10). Please verify TFVC results.\n";
					}
...
#pragma omp critical
					{
						std::cout << "TFVC -- Atom pair: " << a << ", " << b << ", Ratio: " << chi[a * ncen + b] << "\n";
					}
...
#pragma omp for schedule(dynamic, 16) nowait
		...
		for (int i = 0; i < ncen; i++) {
			...
			if (debug)
				file << "Atom " << std::setw(3) << i << " charge: " << charges[i] << "\n";
		}
