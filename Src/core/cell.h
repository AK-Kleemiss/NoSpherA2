			...
			for (int k = 0; k < cell_keywords.size(); k++)
			{
				if (line.find(cell_keywords[k]) != std::string::npos)
				{
					...
					case 6:
						...
						break;
					default:
						file << "This is weird... should never get here... aborting!\n";
						return false;
					}
					found[k] = true;
				}
		...
		while (!cif_input.eof() && !symm_found)
		{
			...
			if (line.find("loop_") != std::string::npos)
			{
				...
				while (line.find("_") != std::string::npos)
				{
					getline_universal(cif_input, line);
					if (debug)
						file << "line in loop field definition: " << line << "\n";
					...
					{
						if (debug)
							file << "I don't think this is the symmetry block.. moving on!\n";
						count_fields = 0;
						break;
					}
					count_fields++;
				...
				while (line.find("_") == std::string::npos && line.length() > 3 && count_fields != 0)
				{
					...
					{
						if (debug)
							file << "This is a new symmetry operation!\n";
						for (int x = 0; x < 3; x++)
							for (int y = 0; y < 3; y++)
