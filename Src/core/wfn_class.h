...
public:
    ...
    void push_back_cartesian_shell(const int mo, const int l, const vec2& shell, const std::vector<primitive>& prims, const int start, const int size, const int* order = nullptr, const double* scale = nullptr);
    ...
    /** Get atom ID. */
    const atomID get_id_for_atom(const int nr) { return atoms[nr].get_ID(); }
    ...
    bool push_back_atom(const std::string& label, const double& x, const double& y, const double& z, const int& charge, const atomID& ID= atomID());
    ...
    const void computeELIELF(const d3& PosGrid, double& Elf, double& Eli) const;
