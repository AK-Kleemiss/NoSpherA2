...
double cube::ewald_sum(const int kMax, const double conv) {
	...
	for (int i = 0; i < 3; i++) {
		std::cout << std::setw(10) << cell_vectors[i][0] << std::setw(10) << cell_vectors[i][1] << std::setw(10) << cell_vectors[i][2] << "\n";
	}
	...
	for (int i = 0; i < 3; i++) {
		std::cout << std::setw(10) << reciprocalLattice[i][0] << std::setw(10) << reciprocalLattice[i][1] << std::setw(10) << reciprocalLattice[i][2] << "\n";
	}
...
#pragma omp parallel for reduction(+:realSpaceEnergy)
	...
	for (int k_vec = 1; k_vec <= kMax; k_vec++) {
		double temp = 0;
		pb = new ProgressBar(8ULL * k_vec * k_vec * k_vec, 60, "-", " ", "Reciprocal-space " + std::to_string(k_vec));
