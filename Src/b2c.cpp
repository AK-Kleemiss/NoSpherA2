...
bool b2c(cube &cub, vector<atom> &atoms, bool debug, bool bcp)
{
    ...
    CP.resize(cub.get_size(0));
    for(int i=0; i<cub.get_size(0); i++) {
    	CP[i].resize(cub.get_size(1));
    	for(int j=0; j<cub.get_size(1); j++){
        	CP[i][j].resize(cub.get_size(2));
    		for (int z=0; z<cub.get_size(2); z++)
    			CP[i][j][z]=0;//	not yet assigned a CP
    	}
    }
    ...
    vector <int> nrs;
    vector < vector < vector <bool> > > border;
    		...
    		for (int z=0; z<cub.get_size(2); z++){
    			if (CP[x][y][z]==0){
    				...
    				do{
    					...
    					if(xs<cub.get_size(0) && ys<cub.get_size(1) && zs<cub.get_size(2)) Max.value=(cub.get_value(xs,ys,zs));
    					else Max.value=0.0;
    					if(Liste[0].size()<=ListeMax) Liste[0].push_back(Max.x);  //only append List when needed check if neccesarry
    					else Liste[0][ListeMax-1]=Max.x;
    					if(Liste[1].size()<=ListeMax) Liste[1].push_back(Max.y);
    					else Liste[1][ListeMax-1]=Max.y;
    					if(Liste[2].size()<=ListeMax) Liste[2].push_back(Max.z);
    					else Liste[2][ListeMax-1]=Max.z;
    					GradMax=0;
    				...
    				for(int i=0; i<3; i++) Liste[i].resize(0); //Revert Liste and free memory
    			}
    		}
    if(bcp){
    	...
    	for(int i=0; i<cub.get_size(0); i++){
    		...
    		for(int j=0; j<cub.get_size(1); j++){
    			...
    			for(int k=0; k<cub.get_size(2);k++) border[i][j][k]=false;
    		}
    	}
    	for(int x=0; x<cub.get_size(0); x++)
    		for(int y=0; y<cub.get_size(1); y++)
    					...
    					for (int iy=y-1; iy<y+2; iy++)
    						for (int iz=z-1; iz<z+2; iz++){
    							...
    							if (CP[ix][iy][iz]!=CP[x][y][z]){
    								border[x][y][z]=true;
    								int found=false;
    								for(int i=0; i<neighbours[CP[x][y][z]].size(); i++) if(neighbours[CP[x][y][z]][i]==CP[ix][iy][iz]) found=true;
    								if(!found) neighbours[CP[x][y][z]].push_back(CP[ix][iy][iz]);
    							}
    						}
    			...
    			for(int x=0; x<cub.get_size(0); x++)
    				for(int y=0; y<cub.get_size(1); y++)
    					for(int z=0; z<cub.get_size(2); z++){
    						...
    						if(!border[x][y][z]) continue;
    						if(CP[x][y][z]!=b) continue;
    						bool found=false;
    						for (int ix=x-1; ix<x+2; ix++)
    							for (int iy=y-1; iy<y+2; iy++)
    								for (int iz=z-1; iz<z+2; iz++){
    						...
    						if(found&&cub.get_value(x,y,z)>Max.value) Max={x,y,z,cub.get_value(x,y,z)};
    					}
    ...
    vector <double> VOL;
    VOL.resize(iCP);
    double dv = abs( cub.get_vector(0,0)*cub.get_vector(1,1)*cub.get_vector(2,2)-cub.get_vector(2,0)*cub.get_vector(1,1)*cub.get_vector(0,2)
                    +cub.get_vector(0,1)*cub.get_vector(1,2)*cub.get_vector(2,0)-cub.get_vector(2,1)*cub.get_vector(1,2)*cub.get_vector(0,0)
                    +cub.get_vector(0,2)*cub.get_vector(1,0)*cub.get_vector(2,1)-cub.get_vector(2,2)*cub.get_vector(1,0)*cub.get_vector(0,1));
    ...
    if(basins.size()>0){
    	...
    	for (int x=0; x<cub.get_size(0); x++){
    		for (int y=0; y<cub.get_size(1); y++){
	       		...
	       		for (int z=0; z<cub.get_size(2); z++){
	       			bool include=false;
	       			for(int a=0; a<basins.size(); a++) if(basins[a]==CP[x][y][z]) include=true;
