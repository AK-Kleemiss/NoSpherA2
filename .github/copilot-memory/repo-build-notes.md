...
- OCC now builds oneTBB as a shared runtime while keeping OCC libraries static. Package/copy `tbb12.dll`, `libtbb.so*`, or `libtbb.dylib` beside NoSpherA2.
- Do not restore `tbbmalloc_proxy` or `tbbmalloc` linking for NoSpherA2. The allocator-proxy workaround was removed after the OCC DF teardown fix.
- Windows Visual Studio validation should build `Windows\Tests\Tests.vcxproj` with `SolutionDir` set to `...\Windows\`, then run `vstest.console.exe Windows\x64\Release\Tests.dll /Platform:x64`.
