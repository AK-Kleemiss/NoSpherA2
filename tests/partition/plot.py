...
bonds_neutron = [1.086, 1.092, 1.093, 1.099, 1.036, 1.050, 1.038]

bonds_HAR = [1.085, 1.089, 1.074, 1.100, 1.028, 1.033, 1.018]

bonds_TFVC = [1.079, 1.085, 1.086, 1.102, 1.051, 1.053, 1.034]

uncertainties = [0.005, 0.006, 0.006, 0.004, 0.004, 0.004, 0.003]

uncertainties_HAR = [0.007, 0.007, 0.007, 0.006, 0.008, 0.008, 0.008]

uncertainties_TFVC = [0.005, 0.004, 0.006, 0.004, 0.006, 0.006, 0.006]
...
ax.bar(x - width, bonds_neutron, width, yerr=uncertainties, label="Neutron", color="#0a6167", capsize=5)
ax.bar(x, bonds_HAR, width, yerr=uncertainties_HAR, label="HAR", color="#f89c0e", capsize=5)
ax.bar(x + width, bonds_TFVC, width, yerr=uncertainties_TFVC, label="TFVC", color="#5b215f", capsize=5)
ax.set_xlabel("Bond")
...
data = pd.read_table(data_path, sep=r"\s+", header=1)
...
# Plot the data
fig_cm = (12.45 * 1.2, 4 * 2 * 1.2)  # cm
fig_in = (fig_cm[0] / 2.54, fig_cm[1] / 2.54)
fig, axs = plt.subplots(2, 2, figsize=fig_in)
...
axs[1][0].plot(data["x"], weight_C * diff, label="HAR")
# axs[1][0].plot(data["x"], data["B_weight_C"] * diff, label="Becke")
axs[1][0].plot(data["x"], data["TFVC_weight_C"] * diff, label="T-BAR")
...
axs[1][1].plot(data["x"], weight_H * diff, label="HAR")
# axs[1][1].plot(data["x"], data["B_weight_H"] * diff, label="Becke")
axs[1][1].plot(data["x"], data["TFVC_weight_H"] * diff, label="T-BAR")
...
fig.legend(handles, labels, loc="center", bbox_to_anchor=(0.57, 0.11), ncol=1, frameon=True)  # tweak y a bit if needed
